...
from booktrack_fastapi.schemas.authors import Author, AuthorCreate, AuthorList, AuthorUpdate
...
def create_author(data: AuthorCreate, db: Session = Depends(get_session)):
    service = AuthorsService(db)
    return service.create(data)


@router.put('/{author_id}', response_model=Author, status_code=HTTPStatus.OK)
def update_author(author_id: int, data: AuthorUpdate, db: Session = Depends(get_session)):
    service = AuthorsService(db)
    return service.update(author_id, data)


@router.delete('/{author_id}', status_code=HTTPStatus.NO_CONTENT)
def delete_author_by_id(author_id: int, db: Session = Depends(get_session)):
    service = AuthorsService(db)
